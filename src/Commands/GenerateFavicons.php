	...
	class GenerateFavicons extends Command {
		...
		private function cropImageIfNecessary(&$image): bool {
			...
			if (imagesx($image) / (float)imagesy($image) !== 1.0) {
				$this->line('The given image was not rectangular, so it was cropped. Please check the output.',
					'fg=yellow');
		...
		private function createResizedImage($image, $size) {
			...
			if (imagecopyresampled($newImage, $image, 0, 0, 0, 0, $size, $size, imagesx($image), imagesy($image)) ===
				false) {
